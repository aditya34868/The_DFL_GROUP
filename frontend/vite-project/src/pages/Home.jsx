...
export default function Home() {
  ...
  return (
    <>
      {/* HERO SECTION */}
      <section id="home" className="relative flex min-h-[92vh] items-center overflow-hidden bg-white px-6 pt-32 pb-16 md:px-8 lg:px-4">
        ...
        <div className="mx-auto grid w-full max-w-7xl items-start gap-12 md:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <p className="max-w-xl text-base leading-relaxed text-[#64748B] opacity-0 [animation:fadeUp_0.7s_0.2s_ease_forwards]">
              DFL Express delivers time-critical packages, heavy cargo, and global supply chain solutions with real-time GPS precision and guaranteed arrival times.
            </p>
          </div>
          ...
          <div className="w-full max-w-md justify-self-center md:justify-self-end">
            <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-300/50">
              <div className="relative h-52 w-full overflow-hidden">
                <img src={LOGISTICS_IMAGES.heroVan} alt="DFL Express Fleet Vehicles" className="h-full w-full object-cover transition-transform duration-700 hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0B132A]/80 via-transparent to-transparent" />
                <span className="absolute bottom-3 left-4 rounded-md bg-[#E66E19] px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-white">Express Fleet Active</span>
              </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  ...
                  <span className="rounded bg-slate-100 px-2 py-0.5 font-mono text-[11px] font-bold text-[#64748B]">#DFL-88219-EX</span>
                </div>
      ...
      {/* PARTNERS MARQUEE (Seamless Seamless Seamless Infinite Loop) */}
      <section className="overflow-hidden border-t border-slate-200 bg-slate-50/80 py-10">
        <div className="mx-auto mb-6 max-w-7xl px-6 text-center md:px-8">
          <p className="text-xs font-extrabold uppercase tracking-widest text-[#E66E19]">Trusted Integration & Global Carrier Network</p>
        </div>

        <div className="relative flex w-full overflow-hidden select-none [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
          <div className="flex min-w-full shrink-0 items-center justify-around gap-12 sm:gap-16 animate-marquee py-2">
            {partners.concat(partners).map((partner, i) => {
              const IconComp = partner.icon;
              return (
                <div key={`p1-${i}`} className="flex shrink-0 items-center justify-center min-w-[100px]">
                  {IconComp ? (
                    <IconComp 
                      className={`h-9 w-auto ${partner.color || "text-[#0B132A]"} filter drop-shadow-sm transition-transform duration-300 hover:scale-105`} 
                    />
                  ) : (
                    <span className="font-extrabold text-xl tracking-tight bg-gradient-to-r from-[#E66E19] via-amber-500 to-[#0B132A] bg-clip-text text-transparent">
                      {partner.text}
                    </span>
                  )}
                </div>
          ...
          <div aria-hidden="true" className="flex min-w-full shrink-0 items-center justify-around gap-12 sm:gap-16 animate-marquee py-2">
            {partners.concat(partners).map((partner, i) => {
              const IconComp = partner.icon;
              return (
                <div key={`p2-${i}`} className="flex shrink-0 items-center justify-center min-w-[100px]">
                  {IconComp ? (
                    <IconComp 
                      className={`h-9 w-auto ${partner.color || "text-[#0B132A]"} filter drop-shadow-sm transition-transform duration-300 hover:scale-105`} 
                    />
      ...
      <section id="services" className="border-t border-slate-200 bg-white px-6 pt-10 pb-4 md:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <div className="inline-block rounded-md bg-[#E66E19]/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#E66E19]">Our Services</div>
              ...
              <p className="mt-3 max-w-md text-sm text-[#64748B]">
                Tailored logistics services built to handle individual parcels, high-volume e-commerce orders, or enterprise heavy cargo.
              </p>
            </div>
          </div>
      ...
      <section className="border-t border-slate-200 bg-white px-6 pt-4 pb-8 md:px-8 relative">
        <div className="mx-auto max-w-6xl">
          <h2 className="font-display text-3xl font-bold tracking-tight text-[#0B132A] md:text-4xl">How it Works</h2>
          <p className="mt-2 text-slate-600">From booking to payment—manage your shipment with ease and tech-driven precision.</p>

          <div className="mt-6 border-t border-slate-200">
            {working.map((work, i) => (
              <details key={work.name} className="group border-b border-slate-200 rounded-lg transition-all duration-300 hover:bg-slate-50 [&[open]]:bg-slate-50">
                <summary className="grid grid-cols-[auto_1fr_auto] items-center gap-6 py-7 px-4 cursor-pointer list-none [&::-webkit-details-marker]:hidden outline-none">
                  ...
                  <div>
                    ...
                    <div className="hidden [@media(hover:hover)]:grid grid-rows-[0fr] opacity-0 transition-all duration-500 group-hover:grid-rows-[1fr] group-hover:opacity-100 group-hover:mt-2">
                      <p className="overflow-hidden text-sm leading-relaxed text-slate-600 max-w-md">{work.description}</p>
                    </div>
                  </div>
                  <span className="font-display text-lg text-slate-400 transition-all duration-300 group-hover:translate-x-1 group-hover:text-[#E66E19] group-[&[open]]:translate-x-1 group-[&[open]]:text-[#E66E19]">→</span>
                </summary>
                <div className="px-4 pb-7 pl-[calc(1rem+2.25rem)] [@media(hover:hover)]:hidden">
                  <p className="text-sm leading-relaxed text-slate-600 max-w-md">{work.description}</p>
                </div>
              </details>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-7xl bg-white px-4 py-10 md:px-8">
        <div className="relative flex flex-col lg:flex-row overflow-hidden rounded-3xl shadow-xl bg-[#E66E19]">
          <div className="flex flex-col justify-between p-8 md:p-12 lg:w-7/12 text-white">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-orange-200">CLIENT SUCCESS STORIES</span>
            ...
            {current && (
              <div className="my-6 flex items-center gap-2">
                <button onClick={prev} className="h-8 w-8 rounded-full bg-white text-[#E65100] flex items-center justify-center font-bold shadow hover:bg-orange-50 shrink-0">‹</button>
                <div className="bg-white text-[#0B132A] p-5 rounded-2xl shadow-md w-full min-h-[160px] flex flex-col justify-between">
                  <p className="text-xs md:text-sm text-slate-700 font-medium leading-relaxed">"{current.quote}"</p>
                ...
                <button onClick={next} className="h-8 w-8 rounded-full bg-white text-[#E65100] flex items-center justify-center font-bold shadow hover:bg-orange-50 shrink-0">›</button>
              </div>
            )}
          ...
          <div className="relative lg:w-5/12 bg-slate-900 min-h-[350px] lg:min-h-full flex items-center justify-center p-6">
            <img src="https://images.unsplash.com/photo-1578575437130-527eed3abbec?auto=format&fit=crop&w=800&q=80" alt="Cargo Ship" className="absolute inset-0 h-full w-full object-cover opacity-50" />
